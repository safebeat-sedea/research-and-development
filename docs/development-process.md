...
**Procedure vs phase delivery ownership (binding).** Single-PR **hoist** is the **default skill procedure** when route is `pr-breakdown-single` — it keeps the ancestor **`Delivery phases`** ledger coherent. It is **not** a claim that the phase plan is the wrong delivery document for the developer. While **`continuationOwner: phase-planner-agent`**, treat the **phase `.plan.md` path** as the **primary delivery anchor** in user-facing recaps (link the phase file first; ancestor hoist is ledger detail). When the developer wants § 5 **`PR breakdown`** on the phase file, use **`decomposeOnPhasePlan: true`** on inline **`pr-breakdown`**. Agents explaining hoist must **not** say the pointer is "correct by design" for the developer's workflow — state that skills default to ancestor hoist and offer **`decomposeOnPhasePlan`**.
