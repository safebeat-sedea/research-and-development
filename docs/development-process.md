...
- **Sedea rules impact** (mode #3 per-PR § 5) — short bullets for **`.cursor/rules/*.mdc`** in the hosting product repo (what to update with the code) and for **`.sedea/centers/research-and-development/rules/40_maintain-rules.mdc`** (center rule hygiene); agent-primary. The `_None — …_` line is still short-bullet form.
